...
const mBezier = (() => {
	...
	function mBezier(mX1, mY1, mX2, mY2) {
...
const mEasings = (() => {
	...
	// Elastic easing adapted from jQueryUI http://api.jqueryui.com/mEasings/
	...
	let functions = {
		linear: mBezier(0.250, 0.250, 0.750, 0.750)
	}

	for (let type in equations) {
		equations[type].forEach((f, i) => {
			functions['ease'+type+names[i]] = isTypeOf.fnc(f) ? f : mBezier.apply(this, f);
		});
	}
...
function animTo(els, styles = {}, opts = {}) {
	...
	addKeys({ duration: 1000, delay: 0, easing: easing, elasticity: elasticity, fill: 'forwards' }, opts);
...
function normalizeEasing(val) {
	return isTypeOf.arr(val) ? mBezier.apply(this, val) : mEasings[val];
}
